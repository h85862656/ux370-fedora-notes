...
（2026-08-05 依 README 新規則補寫，內容來自當時的實測與設計決策）

| 狀態 | CPU | RAM | 說明 |
| --- | --- | --- | --- |
| 平板模式**關閉**（絕大多數時間） | 0 | 0 | daemon 根本不存在。systemd 只留一個監聽用的 socket，是核心裡的資料結構、不會執行，也不會喚醒 CPU |
| 平板模式**開啟** | 0 | 一個 Python 行程 | daemon 阻塞在 `accept()` 等待連線。**在 Linux 裡睡著等待的行程不會被排到 CPU 上執行**，所以是 0 而不是「很低」。必須活著是因為虛擬裝置的存在與否等同於 fd 有沒有開著 |
| GNOME 擴充套件（按鈕） | 0 | 幾個介面物件 | 活在本來就在跑的 `gnome-shell` 行程裡，**沒有任何計時器**，只在 GNOME Shell 啟動時問一次狀態 |

**續航：沒有可量測的影響。** 整套設計裡沒有任何東西會週期性喚醒 CPU，而週期性喚醒才是背景程式耗電的主因。

這也是為什麼「按鈕狀態同步」被刻意留給 02b 用事件通知處理（見下方「已知限制與後續」）——輪詢版本會讓每次打開系統選單都喚醒 daemon 8 秒，為了一個外觀細節破壞上面這張表，不划算。
